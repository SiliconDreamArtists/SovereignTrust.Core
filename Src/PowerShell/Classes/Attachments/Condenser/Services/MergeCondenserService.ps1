...
class MergeCondenserService {
    ...
    MergeCondenserService([MappedCondenserAttachment]$mappedAttachment, [Conductor]$conductor) {
    ...
    [Signal] InvokeByParameter([object]$Base, [object]$Overlay, [bool]$IgnoreInternalObjects = $true) {
        $signal = [Signal]::new("MergeCondenserService.Invoke-ByParameter")
